...
def match_story_to_thought(story_string)
  word_array = story_string.downcase.split(/\W+/)
  word_array.select!{|word| word if word.length > 2}
  words_to_reject = ['this', 'that', 'these', 'the', 'and', 'have', 'had', 'with', 'from', 'what', 'can', 'but', 'get', 'for', 'then', 'there', 'our', 'all', 'when', 'will', 'ever', 'every', 'are']
  ...
  Thought.all.each do |thought|
    ...
    if counter > 0
      thought_and_word_matches[thought.title] = counter
    end
  end
...
def create_word_array(story_string, thought_string)
  ...
  words_to_reject = ['this', 'isn', 'that', 'these', 'the', 'would', 'wouldn', 'and', 'have', 'having', 'had', 'has', 'haven', 'with', 'from', 'what', 'where', 'can', 'but', 'get', 'got', 'getting', 'for', 'then', 'there', 'our', 'all', 'when', 'will', 'ever', 'every', 'are', 'aren', 'along', 'into', 'just', 'were', 'was', 'not', 'say', 'about', 'after', 'across', 'above', 'out', 'well', 'one', 'didn', 'before', 'behind', 'while', 'how', 'few', 'many', 'much', 'see', 'look', 'like', 'able', 'off', 'should', 'shouldn', 'could', 'couldn', 'only', 'why', 'said', 'most', 'more', 'his', 'her', 'him', 'she', 'they', 'them', 'their', 'here', 'there', 'because', 'who', 'back', 'doing', 'don', 'good', 'around', 'wont', 'else', 'too', 'now', 'been', 'some', 'become', 'wasn', 'use', 'did', 'know', 'through', 'though', 'than', 'then', 'its', 'right', 'left', 'took', 'take', 'does', 'make', 'made', 'within', 'even', 'anything', 'knew', 'thought', 'own', 'really', 'want', 'something', 'tell', 'told', 'each']
...
q = find_story
w = match_story_to_thought(q)
w2 = w.first[0]
arr = create_word_array(q, w2)
z = search_matches(arr)
puts q
puts w2
z.first[0]
